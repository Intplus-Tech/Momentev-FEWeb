...
import type {
  CustomerQuoteRequestListResponse,
...
/**
 * Fetch paginated quote requests for the authenticated customer
 * GET /api/v1/quote-requests/me
 */
export async function fetchCustomerQuoteRequests(
  page = 1,
  limit = 10,
  filters?: CustomerQuoteRequestFilters
): Promise<ActionResponse<CustomerQuoteRequestListResponse>> {
  ...
  return makeAuthenticatedRequest<CustomerQuoteRequestListResponse>(
    `${API_URL}/api/v1/quote-requests/me?${params.toString()}`,
    { method: "GET" }
  );
}

/**
 * Close a quote request (customer only, must be in "new" or "responded" status)
 * POST /api/v1/quote-requests/{quoteRequestId}/close
 */
export async function closeQuoteRequest(
  ...
  return makeAuthenticatedRequest<void>(
    `${API_URL}/api/v1/quote-requests/${quoteRequestId}/close`,
    { method: "POST" }
  );
}
