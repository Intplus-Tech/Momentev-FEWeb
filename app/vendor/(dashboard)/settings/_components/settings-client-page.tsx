...
const vendorTabValues = ["reviews", "security", "support"];

/**
 * SettingsClientPage — full client-side settings UI.
 * Rendered by the Server Component (page.tsx) only after the
 * business_profile permission check passes.
 */
...
function VendorSettingsContent() {
  ...
  const initialTab = useMemo(() => {
    const fromQuery = searchParams.get("tab") ?? "reviews";
    return validTabs.has(fromQuery) ? fromQuery : "reviews";
  ...
  const reviewStats = (() => {
    ...
    const distribution = [5, 4, 3, 2, 1].map((stars) => ({
      stars,
      count: reviewsArr.filter(
        (r: any) => Math.round(r.rating) === stars,
      ).length,
    }));
    const totalCalculated = reviewsArr.length;
    const sumRating = reviewsArr.reduce(
      (acc: number, r: any) => acc + r.rating,
      0,
    );
    const averageCalculated =
      totalCalculated > 0 ? sumRating / totalCalculated : 0;
    return {
